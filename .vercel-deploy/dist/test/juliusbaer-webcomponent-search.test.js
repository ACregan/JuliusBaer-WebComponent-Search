...
describe('JuliusbaerWebcomponentSearch', () => {
    ...
    `);
        ...
        expect(el.shadowRoot?.innerHTML).to.contain('WARNING: Mandatory Attributes Are Missing.');
    });
    ...
    `);
        ...
        searchInput.dispatchEvent(new Event('input', { bubbles: true, composed: true }));
        await el.updateComplete;
        const resultItems = Array.from(el.shadowRoot?.querySelectorAll('.result-item') ?? []);
        expect(resultItems.length).to.equal(2);
        const resultCheckboxes = Array.from(el.shadowRoot?.querySelectorAll('.result-checkbox') ?? []);
        ...
        resultItems[1].focus();
        resultItems[1].dispatchEvent(new KeyboardEvent('keydown', {
            key: ' ',
            code: 'Space',
            bubbles: true,
            composed: true,
    ...
    `);
        ...
        searchInput.dispatchEvent(new Event('input', { bubbles: true, composed: true }));
        await el.updateComplete;
        const rootContainer = el.shadowRoot?.querySelector('#root-container');
        ...
        try {
            ...
            await el.updateComplete;
            const resultsContainer = el.shadowRoot?.querySelector('#results-container');
            expect(el.displayResultsAboveInput).to.equal(true);
            expect(resultsContainer.classList.contains('positionAbove')).to.equal(true);
        }
        finally {
    ...
    `);
        ...
        await el.updateComplete;
        const statusMessage = el.shadowRoot?.querySelector('#status-message');
        const searchInput = el.shadowRoot?.querySelector('#search-input');
        ...
        resolveFetch?.(new Response(JSON.stringify([{ id: 11, name: 'Case' }]), {
            status: 200,
            headers: { 'Content-Type': 'application/json' },
        }));
        await Promise.resolve();
    ...
    it('sets data to an empty array when fetched payload is not an array', async () => {
        window.fetch = (async () => new Response(JSON.stringify({ id: 1, name: 'Not an array' }), {
            status: 200,
            headers: { 'Content-Type': 'application/json' },
        }));
        const el = await fixture(html `
    ...
    `);
        ...
        searchInput.dispatchEvent(new Event('input', { bubbles: true, composed: true }));
        await el.updateComplete;
        const renderedKeys = Array.from(el.shadowRoot?.querySelectorAll('.result-cell-key') ?? [])
    ...
    `);
        ...
        const eventPromise = new Promise(resolve => {
            window.addEventListener('Test_ResultUpdate', (event) => {
                const customEvent = event;
                resolve(customEvent.detail);
            }, { once: true });
        });
        const submitButton = el.shadowRoot?.querySelector('#selection-submit-button');
        submitButton.click();
    ...
    `);
        ...
        el.selectedResults = [];
        await el.updateComplete;
        const selectAllButton = el.shadowRoot?.querySelector('#select-all-button');
    ...
    `);
        ...
        expect(searchInput.value).to.equal('MO');
        expect(el.shadowRoot?.innerHTML).to.contain('results-container');
        const outsideElement = el.parentElement?.querySelector('#outside-element-selector');
        outsideElement.dispatchEvent(new MouseEvent('click', { bubbles: true, composed: true }));
        await el.updateComplete;
        expect(el.shadowRoot?.innerHTML).to.not.contain('results-container');
    });
});
//# sourceMappingURL=juliusbaer-webcomponent-search.test.js.map